...
void NewScene::init(SDLInfo *sdl_info)
{
    ...
    main_transform = std::make_shared<TransformNode>();
    main_transform->left_translate(0.0f, 0.0f);
    ...
    auto trans_0 = std::make_shared<TransformNode>();
    trans_0->left_scale(111.0f, 222.0f);
    trans_0->left_translate(100.0f, 40.0f);


    auto trans_1 = std::make_shared<TransformNode>();
    trans_1->left_rotate(1.0);
    trans_1->left_translate(20.0f, 40.0f);

    root_.add_child(main_transform);

    main_transform->add_child(trans_0);
    main_transform->add_child(trans_1);

    trans_0->add_child(spr_0);
    trans_1->add_child(spr_1);
...
{
    main_transform->left_translate(1.0f, 0.0f);
    scene_state_.reset();
    scene_state_.sdl_info = sdl_info_;
